...
def process_solution(problem: str, participant, file_type, submission_file, timestamp):
    ...
    id = uuid4().hex
    f = open(os.path.join('content', 'tmp', 'sub_run_'+id+'.txt'), 'w')
    f.write(problem.pk)
    f.write(s.pk)
    f.write(file_type)
    ...
    f = open(os.path.join('content', 'tmp', 'sub_run_'+id+'.txt'), 'r')
    ...
    for i in range(len(testcases)):
        if testcases[i].public is False:
